...
# Vasopressors (lowercased substrings) — their use marks clinical deterioration.
VASOPRESSORS = ("norepinephrine", "epinephrine", "vasopressin", "dopamine",
                "phenylephrine", "levophed", "dobutamine")

def compute_deterioration_outcome(stay_row, labs_df, vitals_df, rx_df,
                                   los_median_hours):
    """
    Build a MEANINGFUL bad-outcome label (not "everyone is ICU").
    bad = ANY of:
      - in-hospital death (hospital_expire_flag)
      - length of stay above the cohort median
      - lactate rises across the stay (>= +1.0 mmol/L, last vs first)
      - creatinine rises (>= +0.5 mg/dL, AKI proxy)
      - SpO2 drops below 90 at any point
      - a vasopressor is administered
    Returns (bad: bool, reasons: list[str]) so you can audit the label.
    Each criterion is independently tweakable.   # TODO[MIMIC] tune thresholds.
    """
    reasons = []

    if bool(stay_row.get("hospital_expire_flag", 0)):
        reasons.append("death")

    # length of stay
    try:
        los = _hours_since(stay_row["intime"], stay_row["outtime"])
        if los > los_median_hours:
            reasons.append("los>median")
    except Exception:
        pass

    # lactate / creatinine trends (first vs last value within the stay)
    def _trend(df, canon, col_time="charttime", col_val="valuenum"):
        sub = df[df.get("canon_lab") == canon] if "canon_lab" in df else df.iloc[0:0]
        if len(sub) < 2:
            return None
        sub = sub.sort_values(col_time)
        return float(sub[col_val].iloc[-1]) - float(sub[col_val].iloc[0])

    lac = _trend(labs_df, "lactate")
    if lac is not None and lac >= 1.0:
        reasons.append("lactate_rise")
    cr = _trend(labs_df, "creatinine")
    if cr is not None and cr >= 0.5:
        reasons.append("creatinine_rise")

    # SpO2 below 90 at any point
    if "canon_vital" in vitals_df:
        spo2 = vitals_df[vitals_df["canon_vital"] == "spo2"]["valuenum"]
        if len(spo2) and float(spo2.min()) < 90:
            reasons.append("hypoxemia<90")

    # vasopressor administration
    if "drug" in rx_df:
        drugs = rx_df["drug"].astype(str).str.lower()
        if drugs.apply(lambda d: any(v in d for v in VASOPRESSORS)).any():
            reasons.append("vasopressor")
...
def build_trajectory_for_stay(
    ...
    for _, r in labs_df.iterrows():
        ...
        canon = r["canon_lab"]
        flag = _lab_flag(canon, r["valuenum"])
        events.append(Event(t, "labs", {canon: r["valuenum"]}))
        lab_by_bin.setdefault(b, {})[canon] = (r["valuenum"], flag)
    ...
    for _, r in vitals_df.iterrows():
        ...
        b = round(t / bin_hours) * bin_hours
        canon = r["canon_vital"]
        events.append(Event(t, "vitals", {canon: r["valuenum"]}))
        vit_by_bin.setdefault(b, {})[canon] = r["valuenum"]
    ...
    for _, r in rx_df.iterrows():
        ...
        if t < 0:
            continue
        events.append(Event(t, "intervention", {
            "drug": str(r["drug"]).lower(),
...
def convert(mimic_dir, out_path, max_patients=None, bin_hours=6.0):
    ...
    def _conv(row):
        ...
        return fn(row["valuenum"]) if fn and row["valuenum"] == row["valuenum"] else row["valuenum"]
    ...
    # cohort LOS median (hours) for the length-of-stay criterion
    try:
        los_all = [(_hours_since(r["intime"], r["outtime"]))
                   for _, r in stays.iterrows()
                   if r["intime"] == r["intime"] and r["outtime"] == r["outtime"]]
        los_all = [x for x in los_all if x and x > 0]
        los_median = sorted(los_all)[len(los_all) // 2] if los_all else 1e9
    except Exception:
        los_median = 1e9
    print(f"[mimic] cohort LOS median = {los_median:.1f}h (used for deterioration label)")
    ...
    for _, s in stays.iterrows():
        ...
        if labs_df.empty and vit_df.empty:
            continue
        # MEANINGFUL deterioration label (not "everyone is ICU")
        bad, reasons = compute_deterioration_outcome(s, labs_df, vit_df, rx_df, los_median)
        det_pos += int(bad); det_neg += int(not bad)
        outcomes = {
            "deterioration": bad,                 # ← the Probe C label
            "deterioration_reasons": reasons,
            "death": bool(s.get("hospital_expire_flag", 0)),
            "from_icu": True,                     # provenance, NOT the label
    ...
    print(f"[mimic] deterioration label balance: {det_pos} bad / {det_neg} good")
