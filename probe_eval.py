...
def probe_A_state_recovery(model, examples, vocab, device, encoder):
    ...
    ybar = ybar_num / ybar_den
    # rough R² against mean baseline
    with torch.no_grad():
        for e in examples:
            tgt = torch.tensor(e["target_values"], device=device)
            msk = torch.tensor(e["target_mask"], device=device)
            for i in obs_idx:
                if msk[i] > 0:
                    sse_tot += (tgt[i].item() - ybar) ** 2
    r2 = 1 - (sse / sse_tot) if sse_tot > 0 else float("nan")
    return {"n_labels": int(cnt), "MAE": round(ae / cnt, 4),
            "R2_vs_mean": round(r2, 4)}


def probe_B_future_state(model, examples, vocab, device, encoder, change_thresh=0.1):
    """
    Future-state prediction vs a persistence baseline.

    Reports TWO views:
      (1) ALL observable future labels — persistence is hard to beat here
          because most states don't change over a bin (so "no change" wins).
      (2) CHANGED-ONLY: future labels where the state actually moved from the
          context time by >= change_thresh. This is the fair test of dynamics —
          can the model predict the states that DO change? Persistence is
          trivially wrong on these, so beating it means real learning.
    """
    obs_idx = [e["index"] for e in vocab["states"]
               if e["observability"] in ("lab", "vital")]
    obs_set = set(obs_idx)
    ...
    # build a lookup of context-time state values per (patient, t_context)
    # so we can tell which target states actually changed.
    ctx_state = {}
    for e in examples:
        if e.get("delta_h", 0) == 0:  # same-time snapshot = the context state
    ...
    with torch.no_grad():
        for e in fut:
            ...
            for i in obs_idx:
                ...
                # changed-only: did this state move vs context?
                if base is not None and abs(tgt[i].item() - base[i]) >= change_thresh:
    ...
    if c_cnt:
        out.update({
            ...
            "note": "CHANGED-ONLY is the fair dynamics test; beating persistence "
                    "there ⇒ model predicts real state changes, not just stasis"})
    else:
        out["note"] = ("no states changed >= threshold between bins — 24h states "
                       "are near-static; try longer Δ (48/72h) or smaller bins")
    return out
