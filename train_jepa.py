...
def make_batches(examples, batch_size, device, shuffle=True):
    """MLP path: flat context vectors."""
    idx = list(range(len(examples)))
    if shuffle:
        random.shuffle(idx)
    for i in range(0, len(idx), batch_size):
        chunk = [examples[j] for j in idx[i:i + batch_size]]
        ctx = torch.tensor([e["context"] for e in chunk], dtype=torch.float32, device=device)
        tgt = torch.tensor([e["target_values"] for e in chunk], dtype=torch.float32, device=device)
        msk = torch.tensor([e["target_mask"] for e in chunk], dtype=torch.float32, device=device)
        cnf = torch.tensor([e["target_conf"] for e in chunk], dtype=torch.float32, device=device)
        dlt = torch.tensor([[e["delta_h"]] for e in chunk], dtype=torch.float32, device=device)
        yield ctx, tgt, msk, cnf, dlt


def make_seq_batches(examples, batch_size, device, shuffle=True):
    """Transformer path: padded token sequences + the same targets."""
    idx = list(range(len(examples)))
    if shuffle:
        random.shuffle(idx)
    for i in range(0, len(idx), batch_size):
        chunk = [examples[j] for j in idx[i:i + batch_size]]
        L = max(len(e["seq"]) for e in chunk)
        B = len(chunk)
        ...
        pad_mask = torch.ones(B, L, dtype=torch.bool, device=device)  # True=pad
        for bi, e in enumerate(chunk):
            for ti, tk in enumerate(e["seq"]):
                modality[bi, ti] = tk["modality"]
                feat_id[bi, ti]  = tk["feat_id"]
                value[bi, ti]    = tk["value"]
                t_hours[bi, ti]  = tk["t_hours"]
                pad_mask[bi, ti] = False
        batch_in = {"modality": modality, "feat_id": feat_id,
                    "value": value, "t_hours": t_hours, "pad_mask": pad_mask}
        tgt = torch.tensor([e["target_values"] for e in chunk], dtype=torch.float32, device=device)
        msk = torch.tensor([e["target_mask"] for e in chunk], dtype=torch.float32, device=device)
        cnf = torch.tensor([e["target_conf"] for e in chunk], dtype=torch.float32, device=device)
        dlt = torch.tensor([[e["delta_h"]] for e in chunk], dtype=torch.float32, device=device)
        yield batch_in, tgt, msk, cnf, dlt
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for ctx, tgt, msk, cnf, dlt in batcher(examples, args.batch_size, device):
            z_ctx = model.context_encoder(ctx)
            with torch.no_grad():
                z_tgt = model.target_encoder(ctx)
            z_hat = model.predictor(z_ctx, dlt)
            jepa_loss = (1 - F.cosine_similarity(z_hat, z_tgt.detach(), dim=-1)).mean()

            pred_states = model.state_head(z_ctx)
            w = msk * cnf * (1.0 + 2.0 * obs_dims)
            state_loss = ((pred_states - tgt) ** 2 * w).sum() / w.sum().clamp(min=1)

            loss = jepa_loss + args.state_weight * state_loss
            opt.zero_grad()
            loss.backward()
            opt.step()
    ...
    print("[train_jepa] NOTE: trained on synthetic_osler data — for sanity/pipeline "
          "only. Real claims need real EHR (see MEDICAL_JEPA_ASSESSMENT.md).")
